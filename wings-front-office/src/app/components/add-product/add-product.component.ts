...
export class AddProductComponent implements OnInit {
  ...
  constructor(private token: TokenStorageService, private shopeService: ShopService) { }
  fileI: File | any;
  fileII: File | any;
  fileIII: File | any;
  fileIIII: File | any;
  ...
  selectFiles(event: any): void {
    if (event.target.files.length != 4) {
      alert("vous ne pouvez choisir que 4 images");
  ...
  saveproduit(): void {
    ...
    console.log(data);
    this.shopeService.createProduct(data, this.fileI, this.fileII, this.fileIII, this.fileIIII)
      .subscribe(
        response => {
        ...
        error => {
          ...
          if (error.status == "201") {
            ...
            Swal.fire(
              'Oops!',
              "Votre produit n'a pas pu être enregistrée, veuillez vérifier vos informations!",
              'error'
            )
          }
        });
